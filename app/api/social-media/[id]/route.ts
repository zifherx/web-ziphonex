...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json(
      ApiResponseDto.success(data, "Red social obtenida correctamente")
    );
  } catch (err) {
    console.error(`[GET /api/social-media/${id}] Error:`, err);

    const statusCode =
      err instanceof Error && err.message.includes("no encontrada") ? 404 : 500;

    return NextResponse.json(
      ApiResponseDto.error(
        err instanceof Error ? err.message : "Error al obtener la red social",
        "FETCH_ERROR"
      ),
      { status: statusCode }
    );
...
export async function PUT(
  ...
  try {
    ...
    return NextResponse.json(
      ApiResponseDto.success(data, "Red social actualizada correctamente")
    );
  } catch (err) {
    ...
    return NextResponse.json(
      ApiResponseDto.error(
        err instanceof Error
          ? err.message
          : "Error al actualizar la red social",
        "UPDATE_ERROR"
      ),
      { status: statusCode }
    );
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      ApiResponseDto.success(data, "Red social eliminada correctamente")
    );
  } catch (error) {
  ...
  try {
    ...
    return NextResponse.json(
      ApiResponseDto.success(
        data,
        `Red social ${data.isActive ? "activada" : "desactivada"} correctamente`
      )
    );
  } catch (err) {
